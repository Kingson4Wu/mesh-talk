...
pub fn init_logging(app_handle: &tauri::AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let log_plugin = tauri_plugin_log::Builder::new()
        .targets([
            ...
            Target::new(TargetKind::Folder { path: logs_dir.clone(), file_name: Some("mesh-talk.log".into()) }),
        ])
...
pub fn get_logs_directory(_app_handle: &tauri::AppHandle) -> Result<std::path::PathBuf, tauri::Error> {
    get_mesh_talk_logs_dir().map_err(|e| tauri::Error::from(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))
}

/// Get the current log file path
pub fn get_current_log_file(_app_handle: &tauri::AppHandle) -> Result<std::path::PathBuf, tauri::Error> {

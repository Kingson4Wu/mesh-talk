...
- **Accepted limitations:** `run_broadcast` assumes a socket already configured for its target (the broadcast-enabled UDP socket setup is node-startup wiring in Plan 2); `decode` accepts trailing bytes after a valid announce (harmless — `verify` authenticates the content, and one datagram is one packet).
